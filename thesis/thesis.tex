...
\SI{52}{\percent} (ensemble median; inter-quartile range \SIrange{51}{56}{\percent}).
...
heuristic flag (validation/training RMSE ratio below~2); only Bayesian ridge, at 2.30,
is flagged. For Russia, three of five clear it (XGBoost and Bayesian ridge fail at
3.12). The flags are not exclusions: they identify which models to read with caution,
and the most flagged model (Bayesian ridge) is the one whose headline gap is the
ensemble outlier.
...
with an implied counterfactual mean of \$88.5/bbl against an actual \$108.5. This is
...
\midrule
\textbf{Ensemble median} & \textbf{22.1} & n/a & \textbf{21.4} & \textbf{52.2} & n/a & \textbf{52.7} \\
\textbf{IQR} & \textbf{[12.1, 31.6]} & n/a & n/a & \textbf{[51.2, 55.9]} & n/a & n/a \\
\bottomrule
\end{tabular}
\end{table}
...
ratio; Brent (red) is the largest, ranking first among all 18 placebo units, so the
...
chokepoint premium, since the factor structure is stable enough across 2020--22 and
2024--26 to carry a large positive effect, while the headline magnitude rests on the
within-event ensemble.
...
($-1.3$ to $-7.1$\%), so the large premium is not a contamination artefact. For
...
Our estimate places the 2026 Hormuz chokepoint premium at roughly half of Brent's
no-event level over the first $\sim$3.5 months. The number holds up across all five
...
the na\"ive baselines. The cross-event transfer supports the sign and broad scale rather
than the exact level. We read the estimate as a complement to, not a competitor of, the
...
contamination-bias sign check, the na\"ive-baseline comparison, and the cross-event transfer
...
specifications); (ii) the donor pool (shared 18, permissive 26 for Russia, full 32); and
...
18-donor pool to the contaminated 32-donor pool, because Russia-treated donors get inflated
...
The relationship specification is Brent-based, so a break \emph{at} $T_0$ is ambiguous; the
