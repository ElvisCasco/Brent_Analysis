...
control method} (SCM), using a pool of 18 macro-financial donor series --- metals,
agriculturals, equities, foreign exchange, and rates --- selected to share Brent's
exposure to common global factors while staying off the oil-supply causal path. To
guard against any single modelling choice driving the result, the synthetic is
formed as the median of an \emph{ensemble} of five estimators with deliberately
different inductive biases (convex SCM, augmented SCM, elastic net, gradient-boosted
trees, and Bayesian ridge). The design is first validated on a known disruption ---
the 24 February 2022 Russian invasion of Ukraine --- where it recovers a price path
consistent with the documented \$95$\to$\$130 move, and is then applied
out-of-sample to Hormuz. Over the post-event window to mid-June 2026 we estimate an
average Brent premium of roughly \SI{53}{\percent} (ensemble median; inter-quartile
range \SIrange{52}{57}{\percent}). The estimate survives an in-space placebo test (Brent
ranks first among all placebo units; permutation $p$ at the attainable floor), a
leave-one-donor-out check, a contamination-bias sign analysis, and a comparison
against naïve univariate counterfactuals. We frame the estimate as a reduced-form,
data-driven complement to the structural scenario estimates produced by energy
institutions, and discuss its use as an input to strategic-reserve, monetary, and
fiscal decisions.
\end{abstract}
...
leaving the remainder dependent on the maritime route. A
disruption to this chokepoint --- by blockade, mining, military exchange, or
insurance-driven withdrawal of shipping --- is the single largest unhedged
supply-side risk in the oil market. Periodic near-misses (the 1980s tanker war, the
2019 Abqaiq strike, repeated tensions through the mid-2020s) produced transient
risk-premium spikes without an actual closure. The 28 February 2026 event is the
realisation of that long-anticipated tail risk, and it raises a sharp empirical
question:
...
Answering this requires the \emph{counterfactual} Brent path --- what the price would
have done absent the disruption --- against which the observed path is compared. The
...
Hormuz. The two events also share an 18-donor pool, which enables the strongest single
...
co-movement), so their agreement is itself informative. Methodologically, we extend
the SCM-on-price design of \citet{becker2021} from a regulated retail-fuel market with
...
full donor catalogue, and the mathematics of the structural-break tests.
...
outcomes --- GDP, trade, employment --- including under discrete geopolitical shocks:
...
countries as donors. That design differs from ours in three respects --- the treatment
is an endogenously chosen regulation rather than an exogenous shock, the outcome is an
...
disruption --- has limited direct precedent, which is the methodological gap the thesis
addresses.
...
\emph{reliability} --- how tightly the outcome is coupled to the latent covariates ---
while regression-to-the-mean injects an offsetting bias. Although their setting is
DiD, the lesson transfers directly to SCM: a good pre-period fit can mask, rather than
remove, contamination, and the value of a donor depends on how reliably it loads on
the common factor. We use exactly this reliability logic to split donors by
...
\label{eq:factor}
\end{equation}
$F_t$ collects the five drivers that move Brent in normal times --- the global
industrial cycle, the US dollar, real interest rates, global risk appetite, and China
demand --- each with non-oil proxies. $o_t$ is the object of interest: the oil-supply
and chokepoint component that a Hormuz disruption shifts. Each donor $j$ has its own
factor representation $X_{jt}=\gamma_j'F_t+u_{jt}$ with, crucially, \emph{zero loading
on $o_t$}: a clean donor is exposed to the same global factors as Brent but is not
...
stable --- the basket tracks Brent. When a chokepoint shock fires, $o_t$ jumps for
Brent but not for the basket, the two decouple, and the decoupling \emph{is} the
estimated premium. Donor selection is therefore the economic exercise of spanning
$F_t$ while excluding $o_t$ --- ``choosing identification over fit'', which is why the
energy complex (WTI, refined products, petrocurrencies), despite the highest raw
correlation with Brent, is deliberately excluded: it loads on the very $o_t$ we are
trying to measure.
...
gap because it integrates over the whole path including reversion --- the object SPR
sizing, pass-through, and fiscal buffering act on --- and is less sensitive to
...
might be affected by the war'' from a hand-wave into a measurable, signable diagnostic
(Section~\ref{sec:results}). It also separates \emph{contemporaneous} contamination
...
A note on honesty of labelling: the fifth model is a Bayesian \emph{ridge} regression,
not full Bayesian structural time series. It implements only the regression-on-donors
component $\beta'x_t$ of \citet{brodersen2015} and omits the trend, seasonal, and
spike-and-slab selection components; at our sample size, with no strong weekly
seasonality, the regression term carries most of the signal, but the model is best read
as a Bayesian-flavoured diversifier rather than a state-space causal-impact estimator
...
The outcome is the daily \emph{Europe Brent Spot Price FOB} (EIA series
\code{RBRTEd}), in USD per barrel, downloaded directly from the U.S.\ Energy
Information Administration \citep{eia_brent}. The donor candidates are 32
macro-financial series from Yahoo Finance (futures, ETFs, FX, rates, volatility),
fetched with per-ticker caching so the panel updates incrementally. The
\citet{caldara2022} daily Geopolitical Risk index enters only as a descriptive
overlay, never as a donor, because it spikes endogenously to the events we study.
Table~\ref{tab:sources} summarises the sources; all are public and
redistributable, and the entire panel is rebuilt by a single fetching notebook so
the analysis is reproducible against the latest available data (here, through
15~June~2026 for Brent --- the binding constraint, given the EIA's roughly one-week
publication lag --- and 23~June~2026 for the donors).
...
\midrule
Brent spot (outcome) & EIA \code{RBRTEd} & Europe Brent Spot FOB, USD/bbl, 1987--present\\
...
Donor selection is the heart of identification (Section~\ref{sec:theory}): a series
...
transfer --- we use a single shared 18-donor pool: the Russia strict-clean set, which is
a subset of the Hormuz clean set. One further donor, VIX, is audited clean but removed
by a data-driven rule: its donor--Brent return relationship breaks on 2022-01-07
(\citealp{baiperron1998}; $\sim$7 weeks before the invasion), inside the pre-window, so
it is dropped to keep the factor loadings stable. The energy complex (WTI, refined
...
Table~\ref{tab:donors} lists the 18-donor pool by factor category.

\begin{table}[h]
\centering
\caption{The shared 18-donor pool, by factor category. The full 32-donor pool adds
...
identification, short enough to exclude acute Brent-specific contamination. Following
\citet{abadie2021}, the Russia pre-window starts 2020-07-01, deliberately \emph{after}
the March--May 2020 cluster (OPEC+ collapse, WTI-negative, acute COVID) that would
otherwise teach the synthetic an abnormal factor loading. The Russia post-window is
truncated at 2022-09-30, the last business day before OPEC+'s October 2022 supply cut
(a Brent-specific policy shock that would confound the invasion premium). The Hormuz
post-window extends to the latest available data (2026-06-15).
...
\multirow{3}{*}{Russia 2022}
 ...
 & Pre-window  & 422 & 64.35 & --    & --    & -- \\
 & Post-window & 151 & 108.54 & --   & --    & -- \\
\addlinespace
\multirow{3}{*}{Hormuz 2026}
 & Full sample & 515 & 76.95 & 14.30 & 59.93 & 138.21 \\
 & Pre-window  & 443 & 72.08 & --    & --    & -- \\
 & Post-window & 72  & 106.92 & --   & --    & -- \\
\bottomrule
\end{tabular}
\end{table}
...
a naïve trend-extrapolation counterfactual unreliable (Section~\ref{sec:results}).}
\label{fig:timeline}
\end{figure}

The two pre-windows differ instructively. The Russia pre-period is volatile
(SD~\$24.8, spanning the COVID recovery and the 2021 reflation rally); the Hormuz
pre-period is calmer (SD~\$14.3). As shown below, this difference drives much of the
...
All five estimators (Table~\ref{tab:models}) take the same input --- log donor levels ---
and target log Brent over the pre-window, then project the post-window counterfactual.
...
transfer; (f) a deliberately contaminated set of naïve univariate counterfactuals
...
heuristic \code{val/train}$<2$ flag (only Bayesian ridge, at 2.30, is flagged); for
Russia, three of five clear it (XGBoost and Bayesian ridge fail at 3.12). The flags are
not exclusions: they identify which models to read with caution, and the most flagged
model (Bayesian ridge) is precisely the one whose headline gap is the ensemble outlier.

\begin{table}[h]
\centering
\caption{Walk-forward cross-validation (5 expanding folds, 20-day horizons). \code{val/train}$>2$
is a heuristic overfit flag, not an exclusion rule.}
\label{tab:wfcv}
\small
...
Russia & Augmented SCM  & 0.047 & 0.092 & 1.95 & OK \\
...
Hormuz & Augmented SCM  & 0.043 & 0.073 & 1.72 & OK \\
...
\SI{22.1}{\percent} over the seven-month window (IQR \SIrange{12.1}{31.6}{\percent}),
with an implied counterfactual mean of \$88.5/bbl against an actual \$108.5 ---
...
\SI{52.2}{\percent} over the window to mid-June (IQR \SIrange{51.2}{55.9}{\percent}),
with an actual post-window mean of \$106.9 against an implied counterfactual of
$\sim$\$70.3. The Hormuz IQR is far tighter than Russia's ($\sim$5 vs $\sim$20 pp),
...
Augmented SCM  & 12.1 & $+0.3$ & 11.8 & 52.2 & $-0.1$ & 52.3 \\
Elastic net    & 22.1 & $+0.7$ & 21.4 & 57.1 & $-0.3$ & 57.3 \\
XGBoost        & 31.6 & $+1.8$ & 29.8 & 51.2 & $-1.5$ & 52.7 \\
Bayesian ridge & $-10.1$ & $+0.1$ & $-10.2$ & 43.3 & $-0.0$ & 43.3 \\
\midrule
\textbf{Ensemble median} & \textbf{22.1} & --- & \textbf{21.4} & \textbf{52.2} & --- & \textbf{52.7} \\
\textbf{IQR} & \textbf{[12.1, 31.6]} & --- & --- & \textbf{[51.2, 55.9]} & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Economic reading of the model spread.} The cross-model dispersion is itself
informative and has an economic explanation rather than being noise. Convex SCM, confined
to the simplex, leans on the most stable donors --- the FX block --- as a ``trend
baseline'' and adds soft-commodity deltas (Coffee, Sugar for Russia). The Bayesian ridge,
with unconstrained Gaussian coefficients, loads heavily and with large opposing signs on
currencies; for Russia those currencies were pushed around by the simultaneous, historically
large Fed tightening cycle, a macro factor orthogonal to the invasion, which is why Bayesian
ridge is the Russia outlier (a negative full-window gap) and why it earns the highest
walk-forward overfit flag. This is the expected behaviour of an unregularised linear model
in a pre-period with a strong non-event macro driver, and it is the reason the ensemble uses
the \emph{median} rather than the mean. For Hormuz, whose calmer pre-period lacks an
analogous confound, the five models agree to within $\sim$5 pp.
...
both pools --- its post/pre RMSPE ratio (6.9--10.2) exceeds every donor's --- so the
permutation $p$-value sits at the attainable floor (0.053 in the 18-donor pool, 0.030 in
the 32-donor pool). This is the strongest signal the test can give at this sample size.
For \textbf{Russia}, no model reaches the floor (XGBoost comes closest at $p=0.16$); the
...
(18-donor pool). Floor $=1/(N{+}1)$: 0.053 (18-donor), 0.030 (32-donor).}
\label{tab:placebo}
\small
\begin{tabular}{@{}llrrr@{}}
\toprule
Event & Model & $p$ (18-donor) & $p$ (32-donor) & Brent ratio \\
\midrule
Russia & Convex SCM     & 0.53 & 0.50 & 2.75 \\
Russia & Augmented SCM  & 0.58 & 0.47 & 3.45 \\
Russia & Elastic net    & 0.42 & 0.41 & 4.00 \\
Russia & XGBoost        & 0.16 & 0.15 & 7.69 \\
Russia & Bayesian ridge & 0.63 & 0.53 & 5.92 \\
\addlinespace
Hormuz & Convex SCM     & \textbf{0.053} & \textbf{0.030} & 6.93 \\
Hormuz & Augmented SCM  & \textbf{0.053} & \textbf{0.030} & 9.05 \\
Hormuz & Elastic net    & \textbf{0.053} & \textbf{0.030} & 8.98 \\
Hormuz & XGBoost        & \textbf{0.053} & \textbf{0.030} & 10.12 \\
Hormuz & Bayesian ridge & \textbf{0.053} & \textbf{0.030} & 10.18 \\
\bottomrule
\end{tabular}
\end{table}
...
ratio; Brent (red) is the largest, ranking first among all 18 placebo units --- the
...
window, the ensemble-median synthetic mean (\$88.5) sits within \$3.5 of the STEO Feb-2022
...
estimate is $\sim$31\%. Hormuz \emph{rises then plateaus} (46\%~$\to$~57\%~$\to$~52\%): the
chokepoint premium builds in and holds, the opposite of attenuation; the slight easing to
52\% at the full horizon reflects Brent's partial mid-June pullback, captured by extending
...
estimate tight around its baseline (convex 51.2--59.3, ASCM 50.6--58.0, elastic 52.4--61.9,
XGBoost 52.3--58.5\%); no single donor drives the result. Russia is wider (e.g.\ convex
...
window (Table~\ref{tab:transfer}), the two simplex-anchored models transfer to a
\emph{positive} Hormuz premium with the smallest degradation in pre-fit (convex 25.3\%,
RMSPE 5$\times$ independent; ASCM 4.0\%, 6$\times$), while the signed-regression, tree, and
prior-dominated models do not transfer (negative implied gaps, pre-RMSPE 18--340$\times$).
The transfer therefore corroborates the \emph{sign and broad order of magnitude} of the
chokepoint premium --- the factor structure is stable enough across 2020--22 and 2024--26 to
carry a large positive effect --- while the headline magnitude rests on the within-event
ensemble.
...
Augmented SCM  & 52.2 & 4.0    & 0.066 & 0.417 \\
Elastic net    & 57.1 & $-92.7$ & 0.052 & 2.917 \\
XGBoost        & 51.2 & $-45.6$ & 0.053 & 0.956 \\
Bayesian ridge & 43.3 & $-100.0$ & 0.037 & 22.377 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Naïve counterfactual floor.} Against donor-free univariate counterfactuals
(Table~\ref{tab:naive}), the SCM adds value and, crucially, the naïve methods' bias
\emph{flips sign across events} purely because the pre-window trend sign differs. A linear
trend extrapolates Russia's recovery rally upward and returns an absurd \emph{negative}
...
with the SCM $\sim$13 and $\sim$3 pp above respectively. The SCM --- anchored to donor
...
\begin{table}[h]
\centering
\caption{SCM vs.\ naïve univariate counterfactuals, full-window gap (\%). \code{rw}: random
walk; \code{lin\_trend}: linear pre-trend extrapolation; \code{arima\_rw/mr}: frozen ARIMA.}
\label{tab:naive}
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Event & SCM median & rw\_flat & rw\_drift & lin\_trend & arima\_rw & arima\_mr \\
\midrule
Russia & 22.1 & 8.7  & $-6.9$ & $-4.2$ & 9.2  & 26.5 \\
Hormuz & 52.2 & 48.9 & 49.8   & 70.9   & 48.1 & 48.7 \\
\bottomrule
\end{tabular}
\end{table}
...
($-1.3$ to $-7.1$\%) --- the large premium is not a contamination artefact. For
\textbf{Russia} the full-window bias is large and \emph{positive} (overestimate, +14 to
+22\% for three of four models), concentrated in low-reliability soft commodities whose
...
mostly in the low-$R^2$ (unreliable) donors. The honest, asymmetric conclusion --- which we
state rather than smooth over --- is that contamination is conservative for the focal Hormuz
estimate but inflates the Russia \emph{full-window} number; Russia's magnitude validation
therefore rests on the early/peak window (where the \$95$\to$\$130 move is recovered), not on
the attenuated full-window average.

\subsection{Interpretation}
The data-driven estimate places the 2026 Hormuz chokepoint premium at roughly half of
Brent's no-event level over the first $\sim$3.5 months, robust across five estimators, an
in-space placebo at the inferential floor, leave-one-out, the sign of contamination bias,
and a naïve-baseline floor; the cross-event transfer corroborates its sign and broad scale.
This complements --- rather than competes with --- the structural scenario estimates of energy
institutions: those answer ``what price does an assumed barrel shortfall imply,'' while the
SCM answers ``what price gap does the observed cross-asset co-movement imply.'' For the
policy levers of Section~\ref{sec:lit} --- reserve releases, monetary pass-through, fiscal
buffers --- the integrated, path-level premium is the relevant input, and the estimate here
provides it with an auditable identification argument.
...
$\sim$52\% (IQR 51--56\%), large and positive under every estimator, and it survives the
in-space placebo (Brent first among all units, $p$ at the floor), leave-one-out, the
contamination-bias sign check, the naïve-baseline comparison, and the cross-event transfer
of sign and scale. On the calibration event the same machinery recovers a magnitude
consistent with the documented \$95$\to$\$130 move and within \$3.5/bbl of the EIA's
pre-invasion structural forecast.

The estimate's value is as a reduced-form, data-driven complement to structural scenario
analysis, with an identification argument a reviewer can audit donor by donor.
\textbf{Limitations} are real and we flag them rather than bury them: the Hormuz
post-window is short ($\sim$72 observations), so every inference test is under-powered and
the placebo can only reach its floor; the convex synthetic under-represents Brent's
volatility (ASCM corrects this); the cross-event transfer degrades once the window is
extended through Brent's June pullback; the Bayesian-ridge model is a regression proxy for
BSTS, not the full state-space model; and the estimand is a reduced-form total effect, not a
supply/demand/risk decomposition. \textbf{Natural extensions} are a longer post-window as
data accrue, per-donor delayed-contamination onset dating to sharpen the post-window choice,
and a full BSTS implementation for autocorrelation-aware credible intervals. None of these
changes the central, deliberately simple finding: the observed co-movement of global
markets with Brent implies that the 2026 Hormuz disruption added a large, statistically
distinguishable premium to crude --- on the order of half its no-event price.
...
% ===================================================================
The headline uses the preferred specification (shared 18-donor pool, preferred pre-window,
ensemble median). The full grid varies (i) the pre-window --- preferred, extended
(2020-01/2023-12 start), narrow (2021-01/2025-01 start); (ii) the donor pool --- shared 18,
permissive 26 (Russia), full 32; and (iii) the model --- each of five plus the median. For
Russia the donor-pool sweep is itself a diagnostic: the gap is expected to \emph{shrink}
monotonically from the clean 18-donor pool to the contaminated 32-donor pool, because
Russia-treated donors get inflated by the same shock and pull the synthetic up. The grid is
produced by \code{06\_Ensemble\_Final} and is summarised in \code{data/validation/}.

% ===================================================================
\section{Bayesian ridge vs.\ full BSTS}\label{app:bsts}
% ===================================================================
...
magnitudes, not true posterior inclusion probabilities. We therefore label it
\code{bayesian\_ridge}, not \code{bsts}, and read its placebo $p$-values as rank statistics
...
a break strictly inside a pre-window (before $T_0$, hence not confounded by Brent's own
treatment) triggers the breakpoint-rule exclusion --- which removes VIX (break 2022-01-07).
The relationship spec is Brent-based, so a break \emph{at} $T_0$ is ambiguous; the
