...
chokepoint so consequential is the absence of a redundant route. The Saudi East--West
and UAE Habshan--Fujairah pipelines together divert only a small fraction of this
volume overland---about 2.6~million b/d of spare capacity against the roughly
20~million b/d that transits the strait \citep{eia_hormuz2025}---and no maritime bypass
exists, so the overwhelming majority of Gulf crude has nowhere else to go. This leaves Hormuz the single strongest bottleneck in the
...
strategic-reserve releases are sized against an implied price displacement (the IEA
coordinated the drawdown of some 180~million barrels after the 2022 invasion of Ukraine
\citep{iea2022release}). The monetary-policy decision to look through an oil shock or to
tighten turns on its size and persistence. Fiscal stabilisation in subsidising oil
importers scales with the move. And sovereign hedging programmes price disruption
scenarios against exactly such magnitudes (Mexico's annual ``Hacienda hedge'' is the
largest such programme in the world \citep{jfi2023mexico}). Each lever acts on the trajectory of the effect, not on a peak-day figure.
...
Section~\ref{sec:theory}.1 is the organising device. It makes donor selection an economic
exercise, renders each identifying assumption transparent, and gives the contamination
...
\emph{specification}: the counterfactual's functional form is unknown, so no single
...
% ===================================================================
\section{Donor Audit}\label{sec:donoraudit}
% ===================================================================
...
\begin{table}[h]
\centering
\caption{Walk-forward cross-validation (5 expanding folds, 20-day horizons).}
\label{tab:wfcv}
\begin{threeparttable}
\small
\begin{tabular}{@{}llrrl@{}}
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} A validation/training RMSE ratio above 2 is a heuristic overfit flag, not an exclusion rule.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{table}[h]
\centering
\caption{Headline post-event gaps (\%).}
\label{tab:headline}
\begin{threeparttable}
\small
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} Drift-adjusted $=$ raw gap minus the pre-period gap trend extrapolated over the post-window (Section~\ref{sec:methods}). Ensemble $=$ cross-model median; band $=$ IQR (Q25--Q75).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{figure}[h]
\centering
\caption{Observed Brent versus the synthetic counterfactual.}
\label{fig:paths}
\includegraphics[width=\textwidth]{figures/counterfactual_paths.png}

\smallskip
\begin{minipage}{\textwidth}
\footnotesize \textit{Notes.} Observed Brent is black. The thick blue line is the
ensemble-median synthetic, the thin grey lines are the five individual models, and the
...
\subsection{Donor weights and contributions}
The previous paragraph attributed the model spread to regularisation meeting a confounded
pre-period; the donor weights show which donors that disagreement runs through.
Figure~\ref{fig:heatmap} reports each model's normalised donor weights for both events.
Two features stand out. The pool is \emph{sparse}: every model concentrates on a handful
of donors and leaves most at zero. And the models \emph{disagree on which handful} ---
convex SCM and XGBoost concentrate weight on two to four donors, augmented SCM and the
elastic net spread it across the FX block, and the Bayesian ridge loads currencies the
others ignore. Russia leans on soft commodities (Coffee, Sugar) and the S\&P~500; Hormuz
leans on Gold and the yen.

How a donor contributes sorts into three descriptive roles. \textbf{Trend carriers} track
Brent's slow level path: Coffee, Sugar and the S\&P~500 for Russia (pre-window
level correlations near $0.9$), Gold and Sugar for Hormuz. Because Brent drifted
\emph{down} through 2024--25, the strongest Hormuz trackers (Gold, Coffee) move
\emph{inversely}, so the synthetic reproduces the decline by loading oppositely-moving
donors. \textbf{Variance trackers} follow Brent's day-to-day fluctuation, but here the
ceiling is low: no donor's daily-return correlation with Brent exceeds about $0.30$
(Russia) or $0.17$ (Hormuz), so the synthetic reproduces Brent's \emph{path} far better
than its daily \emph{variance}. \textbf{Late divergers} are clean at \T{} but decouple
from the pool's common factor over the post-window. For Russia the EM and Asian FX block
(CNY, JPY, KRW, INR) diverges most, the delayed discounted-crude and terms-of-trade
channel audited in Section~\ref{sec:donoraudit}; for Hormuz the diverger is Gold itself,
on its post-strike safe-haven bid.

These roles explain the cross-model spread, and hence the headline IQR. The donors the
models disagree on most are precisely the influential ones that also diverge late: Gold
for Hormuz, which carries both the largest consensus weight and the largest across-model
weight dispersion in the pool, and the FX block for Russia. The inter-quartile band is
therefore not random model noise but the models disagreeing over how much to trust a few
influential donors whose post-window paths are idiosyncratic or contaminated. This is the
mechanism behind the two design choices already made: reporting the cross-model median
rather than the mean (robust to one model over-loading a diverging donor), and reading
the Russia full-window gap as the more contaminated of the two estimates. The per-donor
weight, trend, variance, and divergence diagnostics behind this paragraph are tabulated in
