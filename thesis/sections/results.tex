...
In Table~\ref{tab:wfcv} we report walk-forward cross-validation. The pattern matches the two
events' pre-periods: for Hormuz, whose pre-period is calm, four of five models clear the
heuristic overfit flag and the fifth sits only just past it; and for Russia, whose pre-period
...
Table~\ref{tab:placebo} and Figure~\ref{fig:placebo} report the in-space placebo, emphasizing the results in the Hormuz. For Hormuz the result is as strong as the test allows: Brent ranks first among all
...
RMSPE as well, which widens the placebo distribution and lowers the test's power, being this behaviour a
...
Hormuz estimate tight around its baseline across all models (convex 48.3 to 56.4, ASCM
...
\begin{figure}[h]
\centering
\caption{SCM vs.\ donor-free na\"ive univariate counterfactuals.}
\label{fig:naive}
...
directional diagnostic, not a structural correction, because it does not prove that the donors are
clean; it is asking a narrower question instead: If some contamination remains despite the audit and the
...
For
Russia the sign reverses and the bias is large and positive, on the order of
...
low-reliability group rather than in the donors that track the common factor well. 
...
in-space placebo reaching its attainable floor as the strongest single result. 
...
different parts of a policy response. 

For the levers set out in Section~\ref{sec:intro},
