...
Russia 2022 the ensemble-median premium is \SI{22.0}{\percent} over the
seven-month window (IQR \SIrange{10.9}{30.8}{\percent}), which implies a counterfactual
mean of \$88.6/bbl against an actual \$108.5, consistent with the documented
\$95$\to$\$130 move and its partial reversion by September. For Hormuz 2026 the
ensemble-median premium is \SI{53.1}{\percent} over the window to late June (IQR
\SIrange{48.3}{53.4}{\percent}), with an actual post-window mean of \$105.1 against an
implied counterfactual near \$68.7. The Hormuz band is far tighter than Russia's, about
...
\toprule
& \multicolumn{2}{c}{Russia 2022 (7 mo)} & \multicolumn{2}{c}{Hormuz 2026 ($\sim$3.8 mo)}\\
...
\midrule
\textbf{Ensemble median} & \textbf{22.0} & \textbf{21.2} & \textbf{53.1} & \textbf{54.2} \\
\textbf{IQR} & \textbf{[10.9, 30.8]} & n/a & \textbf{[48.3, 53.4]} & n/a \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} Drift-adjusted $=$ raw gap minus the pre-period gap trend
extrapolated over the post-window (see Section~\ref{sec:methods}). Ensemble $=$
...
\begin{figure}[h]
\centering
\caption{Observed Brent vs.\ the synthetic counterfactual.}
\label{fig:paths}
\includegraphics[width=\textwidth]{figures/counterfactual_paths.png}

\smallskip
\begin{minipage}{\textwidth}
\footnotesize \textit{Notes.} Observed Brent is the black line. The thick blue line is the
ensemble-median synthetic, thin grey lines are the five individual models, and the shaded
region after $T_0$ (dashed red) is the estimated premium.
\end{minipage}
\end{figure}

The cross-model dispersion has an
economic explanation rather than reflecting noise, and this is the one place where the
individual models matter. Convex SCM, confined to the simplex,
...
large Fed tightening cycle, a macro factor that has nothing to do with the invasion, and
that confound is why the Bayesian ridge becomes the Russia outlier, with a negative full-window
gap, and why it carries a high walk-forward flag. It is the expected behaviour of an
unregularised linear model in a pre-period containing a strong non-event driver, and it
...
reports each model's normalised donor weights for both events. The pool is sparse, so most models concentrate on a handful of donors and leave
the rest at or near zero, but they disagree on which handful. For example, Convex SCM and XGBoost
...
comparable selection weights (Appendix~\ref{app:formulas}). 
\end{minipage}
\end{figure}
...
\begin{itemize}
\item \emph{Trend carriers} carry the largest donor weights, so their combination sets
  ...
  straight-line co-movement.

  For Russia the carriers (Coffee, Sugar and the S\&P~500) climb
  alongside Brent, while for Hormuz, Brent drifts down through 2024--25 while Gold and Coffee
...
\item \emph{Late divergers} sit on the pool's common factor at \T{} and then peel away over the post-window, shown in the bottom row where each line leaves zero after the cutoff. The role is defined by this behaviour and then attributed to a cause donor by
  donor, and the row plots only the divergers that also carry weight, since a donor that
  decouples but is never loaded cannot move the estimate.

  For Russia the weight sits on the
...
the yen sits almost flat as a level anchor.

Late divergers (bottom) are the weighted donors
...
omitted because the models do not load them. The per-donor weights and diagnostics behind
both rows are in Appendix~\ref{app:donorweights}.
\end{minipage}
\end{figure}
...
A credible estimate must clear the three levels of Section~\ref{sec:methods}, pre-event fit,
donor identification, and inference, read here in that order.
...
pre-period inflates validation error. For this analysis, the flags do not exclude any model, we only want to mark which
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} A validation/training RMSE ratio above 2 is a heuristic overfit
...
The donor-level cleanliness battery that anchors this level, defending the no-interference
(SUTVA) assumption, is run before any model is fit and is reported with the donor audit
(Section~\ref{sec:donoraudit}). The three diagnostics that act on the synthetic as a whole
follow, the sign of any residual contamination, how it accrues with horizon, and whether the
factor structure is stable enough to carry across regimes.

\textit{Model-level contamination bias.} This diagnostic is naive and directional, and does
not prove the donors clean. It asks instead which way any contamination remaining after the
audit and break tests would bias the estimate, and equation~\eqref{eq:bias} with a Brent-free
reference answers it. For Hormuz, where every other check already supports a stable estimate,
any residual contamination carries a small negative sign, between about $-1$ and $-8\%$, so if
the estimate is biased at all it is biased downward and the headline premium is conservative.

For Russia the sign reverses and the bias is large and positive, about $+13$ to $+21\%$. The
high-price, risk-off environment of 2022 depressed the demand-cyclical donors, the equities,
metals, and soft commodities, relative to the safe-haven and currency donors, so the synthetic
sits too low and the gap reads too high. Splitting donors by pre-window factor $R^2$ confirms
the bias concentrates in the low-reliability donors rather than those that track the common
factor well. Contamination is therefore conservative for the focal Hormuz estimate but
inflates the Russia full-window number, so Russia's magnitude validation rests on the early
and peak window, where the \$95$\to$\$130 move is recovered, not on the attenuated full-window
average.

\textit{Post-window horizon sensitivity.} Figure~\ref{fig:sensitivity} reports the gap at the
1, 2, 3, 6-month, and full horizons. Russia declines with horizon, from about 31\% to 22\%,
the signature of delayed contamination as second-round war channels switch on through 2022 and
pull the synthetic up, so the full-window figure is conservative and the least-contaminated
one-month estimate is about 31\%. Hormuz instead rises and then eases, from about 46\% to 59\%
and back to 53\%. The chokepoint premium builds in and holds, and the slight easing at the full
horizon reflects Brent's partial mid-June pullback that appears once the window extends to the
latest data. The same diagnostic declining for Russia and rising for Hormuz shows it is not a
mechanical artefact.
...
\textit{Cross-event weight transfer.} As an exploratory probe rather than a validation
...
convex SCM transfers to a clearly positive Hormuz premium, at 23.4\% with a degraded but
...
The models that fail hardest are the ones that load most on currencies, and
the dollar and emerging-market FX relationships shifted most between the two regimes.
Extending the Hormuz window through the June pullback widens the regime gap further, so we
...
\begin{table}[h]
\centering
\caption{Cross-event weight transfer: Russia-fitted weights applied to Hormuz.}
\label{tab:transfer}
\begin{threeparttable}
\small
\begin{tabular}{@{}lrrrr@{}}
...
Table~\ref{tab:placebo} and Figure~\ref{fig:placebo} report the in-space placebo, with the
focus on Hormuz. For Hormuz the result is as strong as the test allows. Brent ranks first among all
placebo units under every model and both donor pools, with a post/pre RMSPE ratio between
6.7 and 9.9 that exceeds every donor's, so the permutation $p$-value sits at the
attainable floor, 0.050 in the 19-donor pool and 0.029 in the 32-donor pool. For Russia no model reaches the floor, and XGBoost comes closest at $p=0.20$. This is because the volatile 2020--22 pre-period gives the donors large pre-period
...
\addlinespace
Hormuz & Convex SCM     & \textbf{0.050} & \textbf{0.029} & 6.73 \\
Hormuz & Augmented SCM  & \textbf{0.050} & \textbf{0.029} & 8.20 \\
Hormuz & Elastic net    & \textbf{0.050} & \textbf{0.029} & 8.27 \\
Hormuz & XGBoost        & \textbf{0.050} & \textbf{0.029} & 9.89 \\
Hormuz & Bayesian ridge & \textbf{0.050} & \textbf{0.029} & 9.92 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} Floor $=1/(N{+}1)$: 0.050 (19-donor pool), 0.029 (32-donor pool).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\textit{Leave-one-donor-out.} Dropping each weighted donor in turn leaves the focal
Hormuz estimate tight around its baseline across all models (convex 48.3 to 56.4, augmented SCM
51.6 to 57.4, elastic 52.0 to 58.8, XGBoost 48.8 to 54.8\%), so no single donor drives the
...
donors, the same concentration that drives the contamination finding above. Read that way,
...
\subsubsection{Corroboration and baselines}
These two checks sit outside the three validation levels, one bringing an independent method
to bear on the magnitude, the other a donor-free floor the SCM should beat.

\textit{External (Russia).} Because Russia is a known event, we
validate its magnitude against the EIA's last two pre-invasion Short-Term Energy Outlook
(STEO) forecasts, issued before 24 February 2022. These forecasts are an independent structural
...
\textit{Naive floor.} We compare the SCM against donor-free univariate
counterfactuals built only from Brent's own past (Figure~\ref{fig:naive}, with full-window
gaps in Table~\ref{tab:naive}), and the comparison yields three findings.

First, the SCM adds value, because it sits about 13 points above
the trend-free random-walk null for Russia and about 7 points above it for Hormuz, so the
donor machinery extracts something that extrapolating Brent's own series cannot.
...
extrapolates Hormuz's mild downtrend and inflates the gap to about 68\%. A counterfactual
...
The SCM leans high, as the contamination analysis above confirms, and the trend
...
\begin{figure}[h]
\centering
\caption{SCM vs.\ donor-free naive univariate counterfactuals.}
\label{fig:naive}
\includegraphics[width=\textwidth]{figures/naive_baselines.png}

\smallskip
\begin{minipage}{\textwidth}
\footnotesize \textit{Notes.} Observed Brent (black) against the SCM synthetic (red dashed,
ensemble) and five counterfactuals built only from Brent's own pre-window: linear pre-trend
extrapolation (\texttt{lin\_trend}), drifted random walk (\texttt{rw\_drift}), the driftless
random walk / ARIMA$(0,1,0)$ carry (\texttt{rw\_flat}), the best-AIC ARIMA$(1,0,0)$
(\texttt{arima\_rw}), and an imposed-mean-reversion ARIMA (\texttt{arima\_mr}).

The dotted line marks $T_0$, and the naive lines are projected over the post-window. The
best-AIC ARIMA collapses to the driftless random walk (\texttt{rw\_flat}) in both events.
\end{minipage}
\end{figure}

\begin{table}[h]
\centering
\caption{SCM and donor-free naive counterfactuals, full-window gaps (\%).}
\label{tab:naive}
\begin{threeparttable}
\small
\begin{tabular}{@{}lrr@{}}
\toprule
Counterfactual & Russia 2022 & Hormuz 2026 \\
\midrule
SCM (ensemble median) & 22.0 & 53.1 \\
\addlinespace
Driftless random walk (\texttt{rw\_flat}) & 8.7 & 46.1 \\
Drifted random walk (\texttt{rw\_drift}) & $-6.9$ & 47.0 \\
Linear trend (\texttt{lin\_trend}) & $-4.2$ & 67.9 \\
Best-AIC ARIMA (\texttt{arima\_rw}) & 9.2 & 45.3 \\
Mean-reverting ARIMA (\texttt{arima\_mr}) & 26.5 & 45.9 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Notes.} Full-window post-event gap in percent, SCM as the ensemble median. The
naive counterfactuals are built from Brent's own pre-window only. The best-AIC ARIMA reduces
to the driftless random walk in both events.
\end{tablenotes}
\end{threeparttable}
\end{table}

\subsection{Interpretation}
The 2026 Hormuz chokepoint premium is roughly \SI{53}{\percent} of Brent's no-event level
over the first nearly four months, and the estimate is stable. It holds across all
...
regime, never for its level. Taken together, the battery describes a Hormuz premium that is
large, stable across estimators, conservatively biased if anything, and driven by no single
donor or model.
