...
admits its own factor representation,
\begin{equation}
X_{jt}=\gamma_j'F_t+u_{jt},
\label{eq:donor}
\end{equation}
with, by construction, zero loading on $o_t$, so a clean donor is exposed to the same global
factors as Brent but is not physically routed through oil supply. A synthetic control is then
a weighted donor basket,
\begin{equation}
\hat p_t=\sum_j w_j X_{jt},
\label{eq:synth}
\end{equation}
and the logic of the method follows directly from \eqref{eq:factor}.
...
\begin{enumerate}
\item \emph{Common-factor spanning (pre-period fit).} The donor basket must reproduce
...
\item \emph{Donor cleanliness (SUTVA / no interference).} No donor may carry the
...
\item \emph{Regime stability.} The loadings $\lambda,\gamma_j$ must hold across the
