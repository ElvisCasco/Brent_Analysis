...
inverse form. Crypto enters the candidate set through Bitcoin, a digital risk-asset and
global-liquidity proxy on that same risk-appetite factor, though the per-event audit excludes
it for Russia on the sanctions-evasion channel that opened in March 2022
(Appendix~\ref{app:donoraudit}). None of the retained series is itself set by oil.
...
Stage 3 keeps only the series clean for both events at once, so the same basket is a
