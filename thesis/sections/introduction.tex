...
The Strait of Hormuz is the narrowest binding constraint in the actual global oil system.
...
exists, so the majority of Gulf crude has nowhere else to go. This leaves Hormuz the single strongest bottleneck in the
...
the majority of internationally traded crude is settled; also, we use its spot quotation
...
rather than a single number. The applications drive this choice for several reasons. First, emergency
...
\citep{iea2022release}); second, the monetary-policy decision to look through an oil shock or to
tighten turns on its size and persistence; third, fiscal stabilisation in subsidising oil
importers scales with the move; and fourth, sovereign hedging programmes price disruption
scenarios against exactly such magnitudes (Mexico's annual ``Hacienda hedge'' is the
largest such programme in the world \citep{jfi2023mexico}). Each lever acts on the trajectory of the effect, not on a peak-day figure, then the object we need is therefore the average gap between observed Brent and the price
...
Standard observational designs cannot estimate properly the causal effect. Brent is a single, globally traded
...
cross-country donor units; however, neither is available here, since Brent is a global crude
...
pre-window. We do not transfer the Russia fit to Hormuz; instead of this, the two events sit in very
different market regimes, and we estimate each one independently. What we try to find is a common pattern: if both are fit
