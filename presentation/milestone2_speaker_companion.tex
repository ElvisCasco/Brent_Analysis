...
\begin{itemize}
  ...
  \item Highlight the two focal events visually: \highlight{Russia 2022} (calibration) and \highlight{Hormuz 2026} (focal).
\end{itemize}

\subsection{If asked}
\begin{itemize}
  \item \textit{Why these two events specifically?} Both are large, well-measured, recent geopolitical disruptions. Russia 2022 is the most recent large oil shock with a long enough post-window to be well-measured; Hormuz 2026 is the focal event of interest.
...
\subsection{Talking points}
\begin{itemize}
  \item Pre-event windows: Russia 2020-07 $\to$ 2022-02 (421 obs); Hormuz 2024-06 $\to$ 2026-01 (423 obs). Post-event: 151 obs Russia / $\sim$59 obs Hormuz.
...
\begin{itemize}
  ...
  \item \textit{What if a clean donor is actually treated?} The model-agnostic battery in 01.5\_Donor\_Cleanliness checks this empirically. Russia perfectly identifies 20/33 cases between audit and tests; Hormuz 32/33. Discrepancies are concurrent macro events (CNY's China zero-COVID), not Russia-treatment.
...
\subsection{Talking points}
\begin{itemize}
  \item Ensemble paths (left Russia, right Hormuz): all 5 model synthetics overlay observed Brent in the pre-event window, then fan out post-event. Visual evidence of pre-event fit AND post-event divergence.
  \item Walk-forward CV table: \highlight{train and val RMSE per model}. Convex SCM val/train ratio is 1.16 Russia / 1.26 Hormuz (clean). ASCM 2.04 Russia (just over 2$\times$ heuristic threshold). XGBoost 3.00 Russia and Bayesian Ridge 3.07 Russia are the overfit-flagged cases.
...
\begin{itemize}
  ...
  \item Implied ATTs: SCM-based 25.3\%, STEO-Feb-22-based 27.8\%, STEO-Jan-22-based 41.9\% (further in time = larger gap).
\end{itemize}
...
\begin{itemize}
  ...
  \item Russia validation: matches EIA STEO to within \$3.98/bbl. Hormuz: validated by in-space placebo at the floor under both pools across all 5 models, plus cross-event weight transfer (appendix).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Appendix A3 --- donor SUTVA cleanliness:} permutation mean-shift test results (Chow 1960 hypothesis + Lehmann \& Romano 2005 permutation + Phipson \& Smyth 2010 correction). Russia 1/33 flagged (CNY); Hormuz 1/33 (IronOre). Audit agreement: Russia 20/33, Hormuz 32/33.
...
\begin{itemize}
  ...
  \item Russia pre-event window: 421 obs / Hormuz pre-event: 423 obs.
