...
\begin{frame}[shrink=5]{Validation results --- in-space placebo}
  ...
  \begin{itemize}\setlength\itemsep{0.1em}
    ...
    \item \hd{Russia:} not decisively rejected --- only XGBoost reaches floor at 21-donor and fails at 33-donor. Feb-22 macro regime shift moves many donors too; Russia's inferential weight rests on EIA STEO (next slide).
  \end{itemize}

  \vspace{0.2em}
  \smalltext{Other tests (parallel-fit, regime-stability, in-time placebo, LOO, cross-event transfer): appendix.}
  \normalsize
\end{frame}
...
% ---------------------------------------------------------------------
% A2. Pre-event diagnostics — moments, parallel-fit, regime-stability
% ---------------------------------------------------------------------
\begin{frame}[shrink=10]{Appendix --- pre-event diagnostics}
  \footnotesize
  \hd{Moment matching} \src{Abadie 2010}. Mean $|\Delta| < 0.01$ logs for all 10 (model $\times$ event) fits. SD undercoverage is the only material issue: convex SCM $-23\%$ on Russia (donor pool cannot span Brent's pre-period vol $\rightarrow$ ridge correction in ASCM closes the gap).

  \vspace{0.5em}
  \hd{Parallel-fit defence (gap-series slope, \%/yr)} \src{Roth 2022 --- pretest with caution}. Reported, not pass/fail.
  \vspace{0.2em}
  \begin{center}
  \begin{tabular}{@{}l r r@{}}
    \toprule
    \hd{Model}     & \hd{Russia} & \hd{Hormuz} \\
    \midrule
    Convex SCM     & $+10.0$ & $-7.0$ \\
    ASCM           & $+0.3$  & $-1.9$ \\
    Elastic-net    & $+0.9$  & $-1.2$ \\
    XGBoost        & $+3.4$  & $-4.7$ \\
    Bayesian Ridge & $+0.2$  & $-0.1$ \\
    \bottomrule
  \end{tabular}
  \end{center}
  \smalltext{Large slopes (convex SCM both events; XGBoost Russia / Hormuz) flag drift; drift-corrected gap = raw gap $-$ slope $\times$ post-window length (Russia $L \approx 0.58$ yr; Hormuz $L \approx 0.25$ yr).}

  \vspace{0.5em}
  \hd{Regime stability} \src{distance correlation $\Delta$ across pre-period thirds; Sz\'ekely \& Rizzo 2007}: 8 of 21 donors exceed the $0.30$ narrative-flag threshold on each event --- read qualitatively (sub-period $n \approx 140$ confounds drift with sampling noise).
  \normalsize
\end{frame}
...
\begin{frame}{Appendix --- in-time placebo \& LOO}
  \footnotesize
  \hd{In-time placebo} \src{Abadie 2015; Chen \& Yan 2023}. Fake $T_0$ set 6 months before real $T_0$; mean fake-period gap (\%), defaults hparams (avoid val-leakage).
  \vspace{0.3em}
  \begin{center}
  \begin{tabular}{@{}l r r@{}}
    \toprule
    \hd{Model}     & \hd{Russia (2021-08-24)} & \hd{Hormuz (2025-08-01)} \\
    \midrule
    Convex SCM     & $+1.9$  & $-11.9$ \\
    ASCM           & $-4.2$  & $-9.8$  \\
    Elastic-net    & $+7.5$  & $-10.6$ \\
    XGBoost        & $+26.4$ & $-12.4$ \\
    Bayesian Ridge & $-5.3$  & $-12.2$ \\
    \bottomrule
  \end{tabular}
  \end{center}
  \smalltext{Russia fake window contains the late-2021 oil rally and the Jan-2022 invasion runup --- partial real-effect contamination expected. Hormuz fake window has no curated events; the concentrated $-10$ to $-12\%$ across models reflects regime drift the synthetic does not absorb, not model-class failure.}

  \vspace{0.6em}
  \hd{Leave-one-donor-out} \src{Abadie 2015}. Post-event gap stable within $\pm 6$ pp across leave-outs for convex SCM / ASCM / Elastic-net / Bayesian Ridge (no single donor drives the headline); XGBoost more fragile ($\pm 12$ pp).
  \normalsize
\end{frame}
