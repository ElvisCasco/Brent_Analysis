...
% =====================================================================
% COMMENT 2 --- PREFACE: the question and our two-step answer
% =====================================================================
\begin{frame}{Comment 2 --- why the models behaved as they did}
  ...
  \vspace{0.8em}
  \hd{How we answer it --- two steps:}
  \vspace{0.3em}
  \begin{itemize}\setlength\itemsep{0.55em}
    \item[\textcolor{accent}{\hd{1}}] \hd{Behaviour $=$ donor weights.} Read which donors each model leans on.
      This explains \emph{why} there is a gap at all (the weighted donors track common factors, \emph{not} the
      oil-specific shock) and \emph{why} the models diverge on Russia (different donors; one degenerate model).
    \item[\textcolor{accent}{\hd{2}}] \hd{Sign the contamination.} The natural worry --- are those donors
      themselves moved by the treatment (demand destruction)? We split each gap into a clean part and a
      donor-contamination part and \emph{sign} it: conservative for Hormuz (a \hd{lower bound}), positive for
      Russia (why Russia is only a \hd{magnitude check}).
  \end{itemize}

  \vspace{0.5em}
  \smalltext{Take-away: the behaviour is interpretable (donor-driven), and the residual contamination is signed --- it does not inflate the headline.}
\end{frame}
