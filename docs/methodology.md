...
URTH (its factor coverage largely subsumed by SP500) has been dropped as redundant, which pushes the binding constraint back to ~2010-01; the 19-donor pool’s effective eligibility floor is therefore ~2011, comfortably below both focal events.
...
3. **Donor pool differs across events.** Russia uses 19 clean donors after SUTVA exclusions; Hormuz uses 31 (Cotton is M). Pooling forces either the 19-donor intersection (losing factor coverage for Hormuz) or violates SUTVA for Russia.
...
|---|---|---|---|
| **Preferred (shared 19-donor intersection)** | 19 donors | 19 donors | Main results, both events |
| Permissive | 26 donors | n/a | Russia appendix only |
| Full | 32 donors | 32 donors | Both events appendix |

**Why a shared 19-donor pool for both events.** The cross-event weight-transfer validation ([validation.md §5f](validation.md)) is the single strongest test of methodology generalization, and it requires both events to use the *same input vector* — i.e., the same donors. The 19-donor intersection (Russia strict-clean ∩ Hormuz strict-clean = Russia strict-clean, since every Russia-clean donor is also Hormuz-clean by construction) is the natural shared pool. It also yields the cleanest like-for-like comparison: same model, same donors, same number of features → any difference in the post-event gap is attributable to the *event itself*, not to changes in inputs.

**Cost of sharing.** Hormuz loses 12 donors that are Hormuz-clean but Russia-contaminated: Wheat, Corn, Palladium, EUR, DXY, BTC *(heavily treated by Russia)* + Copper, IronOre, Soybeans, EM_Eq, GBP, US10Y *(mildly treated by Russia)*. Factor coverage thins slightly but the remaining categories (metals, agri, equities, FX, rates) stay represented in the 19 (the volatility category — VIX — is retained; see the Battery-C policy note below). The full-pool Hormuz fit is reported in the appendix to quantify what is lost — typically modest because convex-SCM weights are sparse (~5–10 donors get non-trivial weight regardless of pool size).
...
**Selection methodology.** Hand-curated based on SUTVA reasoning + factor coverage (see [donor_catalog.md](donor_catalog.md)); the same 19 donors are fed to *all five models* in the ensemble (model-agnostic candidate pool). Each model assigns its own internal donor importance via convex weights, regression coefficients, permutation importance, or posterior inclusion probability — but the candidate pool is shared, preserving cross-model and cross-event comparability of gap estimates.

**Battery C break tests are informative, not exclusionary (policy).** Beyond the hand audit, the Bai-Perron relationship breakpoint test ([validation.md §5c ii](validation.md)) and the HLT trend-break test flag donors whose donor↔Brent loading or own trend shows a structural break inside the pre-window. These flags **inform** the researcher's judgment but do **not** trigger an automatic pool exclusion. `config.BREAKPOINT_EXCLUDE` is a manual, researcher-curated list and is currently **empty**: pool membership is driven by the qualitative audit, per the SCM "report and interpret" convention. The previously-excluded **VIX** is now **retained** — its only flag was a single full-history relationship break (2022-01-07, the early-January Fed hawkish pivot plus nascent Ukraine-risk repricing), which is a loading-*stability* signal rather than a SUTVA violation, does not reproduce when the test is re-run on the analysis window alone (`scripts/battery_c_analysis_period_robustness.py`), and is immaterial to the estimate (a VIX in/out sensitivity moves the ensemble premium by −0.2 pp for Russia and +0.6 pp for Hormuz; `scripts/vix_sensitivity.py`). The complementary at-$T_0$ mean-shift test (which flags **CNY**) is likewise a flag for economic discussion, not an exclusion, because it cannot separate treatment contamination from coincident macro drift and tests the donor in isolation rather than its Brent relationship.
...
The main results table reports only the **preferred specification** for each dimension (preferred pre-window, shared 19-donor pool, ensemble-median gap). The full grid below is reported as an **appendix robustness table** to demonstrate that the headline estimate does not depend on any single methodology choice.
...
**Russia donor-pool sensitivity is especially diagnostic** (cross-reference §3). The gap estimate is expected to be *largest* with the 19-donor preferred pool (cleanest), *smaller* with the 26-donor permissive pool (mild contamination from M-tier donors pulls the synthetic up), and *smallest* with the 32-donor full pool (heavy contamination from H-tier donors). If this monotonic pattern holds empirically, the SUTVA-driven exclusion logic is empirically confirmed — a free-standing validation of the donor-audit methodology in [donor_catalog.md](donor_catalog.md).
